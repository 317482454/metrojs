(function() {
  ...
  var MtrAddress = require('../util/mtraddress.js');
  var helpers = require('./helpers')


  function rsConvert(address) {
    var addr = new MtrAddress();
    addr.set(address);
    return {
  ...
  function publicKeyToAccountId(publicKey, numeric) {
    ...
    var slice = (helpers.hexStringToByteArray(account)).slice(0, 12);
    var accountId = helpers.byteArrayToBigInteger(slice).toString();

    if (numeric) {
      return accountId;
    }
    var address = new MtrAddress();
  ...
  function signTransactionBytes(data, secretPhrase) {
    ...
    var signed = unsignedBytes.slice(0,104);
    signed = signed.concat(sig);
    signed = signed.concat(unsignedBytes.slice(104 + 64));
